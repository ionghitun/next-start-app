...
try {
    ...
    try {
        ...
        try {
            ...
            console.log(new Date, 'Cannot connect to Redis server on process.env.SOCKET_PORT ' + REDIS + process.env.SOCKET_PORT);
            process.exit(1);
        }
    } catch (err) {
    ...
    console.log(new Date, 'Server failed to run on port ' + process.env.SOCKET_PORT);
    process.exit(1);
}
